...
const HeroImageSphere = () => {
  ...
  const images = [image1, image2, image3, image4, image5];
  const slides = [
    { title: "Entrepreneurship in Action", alt: "Youth practicing poultry farming" },
    { title: "Digital Literacy Workshop", alt: "Facilitator teaching social media skills" },
    { title: "Mentorship Circle", alt: "A successful alum shares their journey" },
    { title: "Health & Wellness Outreach", alt: "Youth organizing a health session" },
    { title: "Community Engagement", alt: "Kesho Angavu leading a clean-up campaign" },
  ];

  // Debugging
  console.log({ images, slides });

  if (!images || !Array.isArray(images) || !slides || !Array.isArray(slides)) {
    throw new Error("Images or slides array is undefined or not an array.");
  }

  if (images.length !== slides.length) {
    throw new Error("The lengths of images and slides must match.");
  }
